...
import { GAME_SETTINGS, DIFFICULTY_REMOVE_COUNTS } from "./constants.js";
...
export const validateUsername = (username) => {
  ...
  if (trimmed.length < GAME_SETTINGS.USERNAME_MIN_LENGTH) {
    return {
      valid: false,
      error: `Username must be at least ${GAME_SETTINGS.USERNAME_MIN_LENGTH} characters`,
    };
  }
  ...
  if (trimmed.length > GAME_SETTINGS.USERNAME_MAX_LENGTH) {
    return {
      valid: false,
      error: `Username must be ${GAME_SETTINGS.USERNAME_MAX_LENGTH} characters or less`,
    };
  }
...
export const validatePassword = (password) => {
  ...
  if (password.length < GAME_SETTINGS.PASSWORD_MIN_LENGTH) {
    return {
      valid: false,
      error: `Password must be at least ${GAME_SETTINGS.PASSWORD_MIN_LENGTH} characters`,
    };
  }
  ...
  if (password.length > GAME_SETTINGS.PASSWORD_MAX_LENGTH) {
    return {
      valid: false,
      error: `Password must be ${GAME_SETTINGS.PASSWORD_MAX_LENGTH} characters or less`,
    };
  }
 ...
 */
export const isValidMove = (board, index, num) => {
  const row = getRow(index);
  const col = getCol(index);
 ...
 */
export const getRemainingNumbers = (board) => {
 ...
 */
export const getCompletedBoxes = (board) => {
  const completed = [];
  for (let b = 0; b < 9; b++) {
    const cells = board.filter(
      (c) => Math.floor(c.row / 3) * 3 + Math.floor(c.col / 3) === b
    );
 ...
 */
export const generateLocalBoard = (difficulty) => {
...
export const triggerConfetti = () => {
  ...
  for (let i = 0; i < 50; i++) {
    ...
    el.style.animation = `confettiDrop ${
      1 + Math.random() * 2
    }s linear forwards`;
    root.appendChild(el);
    pieces.push(el);
 ...
 */
export const sortLeaderboard = (list) => {
