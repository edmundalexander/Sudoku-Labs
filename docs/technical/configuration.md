...
### `public/config/config.example.js` (Public - Committed)
...
### `public/config/config.local.js` (Private - Not Committed)
...
window.CONFIG = {
  GAS_URL:
    "https://script.google.com/macros/s/AKfycbzZg11UDcIZGbwHvrtxb5E2enGspkQnjsBPbCP5Aw6BYP5Jo5cq3JqPr8PHOZgbgn2kOg/exec",
};
```

## How It Works

1. **Frontend loads config** (`index.html`):

   ```html
   <script>
     loadConfig("/config/config.local.js");
   </script>
   ```
...
1. Copy `public/config/config.example.js` to `public/config/config.local.js`
2. Update `public/config/config.local.js` with your actual GAS URL
3. Never commit `public/config/config.local.js`

### For Deployment

Your `public/config/config.local.js` stays private locally:
